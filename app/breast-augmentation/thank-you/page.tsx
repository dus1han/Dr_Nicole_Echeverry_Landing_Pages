...
import { breastAugmentation } from '@/content/breast-augmentation';
...
const WA_MESSAGE = "Hi, I've just requested a Breast Augmentation consultation.";

export default function ThankYouPage() {
  const { successTitle, successBody } = breastAugmentation.booking;
  ...
  return (
    <main className="grain relative grid min-h-screen place-items-center overflow-hidden bg-[linear-gradient(180deg,var(--color-blush-100)_0%,var(--color-blush-50)_55%,var(--color-cream)_100%)] px-6 py-16">
      <AuroraBackground />
      <LeadEvent formLocation={breastAugmentation.slug} />

      <div className="relative z-10 w-full max-w-xl text-center">
        <Link
          href={`/${breastAugmentation.slug}`}
          className="inline-block"
          aria-label={`${site.doctor.name} — back to the Breast Augmentation page`}
        >
          <Image
