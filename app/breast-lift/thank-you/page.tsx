...
import { breastLift } from '@/content/breast-lift';
...
const WA_MESSAGE = "Hi, I've just requested a Breast Lift & Augmentation consultation.";

export default function ThankYouPage() {
  const { successTitle, successBody } = breastLift.booking;
  ...
  return (
    <main className="grain relative grid min-h-screen place-items-center overflow-hidden bg-[linear-gradient(180deg,var(--color-blush-100)_0%,var(--color-blush-50)_55%,var(--color-cream)_100%)] px-6 py-16">
      <AuroraBackground />
      <LeadEvent formLocation={breastLift.slug} />

      <div className="relative z-10 w-full max-w-xl text-center">
        <Link
          href={`/${breastLift.slug}`}
          className="inline-block"
          aria-label={`${site.doctor.name} — back to the Breast Lift page`}
        >
          <Image
